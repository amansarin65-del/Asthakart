...
// These image URLs point to Wikimedia Commons files that are actually the products named below.
const products = [
  ['Sandalwood Incense Sticks','Hindu','₹199','https://commons.wikimedia.org/wiki/Special:FilePath/Incense%20sticks.jpg?width=1200'],
  ['Brass Diya Set','Hindu','₹499','https://commons.wikimedia.org/wiki/Special:FilePath/A%20lit%20Diyo.jpg?width=1200'],
  ['Prayer Beads','Islamic','₹349','https://commons.wikimedia.org/wiki/Special:FilePath/Islam%20Prayer%20Beads.jpg?width=1200'],
  ['Wooden Meditation Mala','Buddhist','₹299','https://commons.wikimedia.org/wiki/Special:FilePath/Buddhist%20prayer%20beads.JPG?width=1200']
];
...
export default function Home(){
  ...
  const chooseCategory=(name)=>{setSelectedCategory(name);setMenuOpen(false);setTimeout(()=>document.getElementById('shop')?.scrollIntoView({behavior:'smooth'}),0)};
  ...
  return <main>
    ...
    {cartOpen && <div className="cartOverlay" role="dialog" aria-modal="true" onClick={()=>setCartOpen(false)}><aside className="cartDrawer" onClick={e=>e.stopPropagation()}><div className="cartHead"><h2>Your cart</h2><button aria-label="Close cart" onClick={()=>setCartOpen(false)}><X/></button></div>{cart.length===0?<div className="emptyCart"><ShoppingBag size={42}/><p>Your cart is empty.</p><button onClick={()=>setCartOpen(false)}>Continue shopping</button></div>:<><div className="cartItems">{cart.map((p,i)=><div className="cartItem" key={`${p[0]}-${i}`}><img src={p[3]} alt=""/><div><strong>{p[0]}</strong><small>{p[2]}</small><button onClick={()=>removeFromCart(i)}>Remove</button></div></div>)}</div><button className="checkout">Proceed to checkout</button></>}</aside></div>}
  </main>
}
